...
var ObsidianProjectFile = Class.$extend({

    __classvars__: {
        FORMAT_JSON:         0,
        FORMAT_JSON_DEFLATE: 1,
        VERSION: 1,
        MIMETYPE: "application/x-obsidian-project",

        isObsidianProjectFile: function (buffer) {
            if (buffer.length <= 51) {
                return false;
            }
            if (buffer.toString("ascii", 0, 4) !== "WPRJ") {
                return false;
            }
            // Maybe this check is too strict
            if (buffer.length !== buffer.readUInt32BE(21) + buffer.readUInt32BE(30) +
                buffer.readUInt32BE(39) + buffer.readUInt32BE(47) + 51) {
                return false;
            }
            return true;
        }
    ...
    __init__: function (obsidianProjectFile) {
        this.$data.version = 1;
        ...
        if (typeof obsidianProjectFile == "string") {
            this._loadData64Url(obsidianProjectFile);
        } else if (obsidianProjectFile instanceof Buffer || obsidianProjectFile instanceof Uint8Array) {
            this._loadBlob(obsidianProjectFile);
        }
    },
    ...
    addBlobFromData64Url: function (data64, blobId, options) {
        options = options || {};
        var blob = Buffer.from(data64.split(",")[1], "base64");
        if (!options.mime) {
    ...
    _loadBlob: function (blob) {
        if (!this.$class.isObsidianProjectFile(blob)) {
            throw new Error("NotAnObsidianProjectFile");
        }
    ...
    _loadData64Url: function (data64) {
        this._loadBlob(Buffer.from(data64.split(",")[1], "base64"));
    },
...
module.exports = ObsidianProjectFile;
